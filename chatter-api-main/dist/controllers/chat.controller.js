...
const createChat = (req, res, next) => {
    ...
    if (!name || !image) {
        const statusError = new StatusError_1.StatusError('Chat requires a name and an image to be created', 400);
        return next(statusError);
    }
    try {
...
const sendMessage = (req, res, next) => {
    ...
    try {
        ...
        if (!chat) {
            const statusError = new StatusError_1.StatusError('Could not find user chat', 404);
            return next(statusError);
        }
        const { message } = req.body;
        if (!message) {
            const statusError = new StatusError_1.StatusError('Must provide a message to send', 400);
            return next(statusError);
        }
